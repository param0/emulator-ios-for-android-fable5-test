//! JNI entry points (`libios_emu_jni.so`).
//!
//! Marshalling is deliberately simple: the app list crosses the boundary as a
//! JSON string (parsed by the Kotlin `AppManager`), icon bytes as a `byte[]`, and
//! a launch returns the process exit code as an `int`. This keeps the FFI
//! surface to four functions and avoids constructing Java objects from Rust.
//!
//! Method names follow the JNI mangling for
//! `com.iosemu.emulator.emu.EmulatorBridge`.
...
/// Install the logcat logger. Called once from `EmulatorBridge.<clinit>`.
#[no_mangle]
...
/// Returns the guest process exit code (negative on host-side failure).
#[no_mangle]
pub extern "system" fn Java_com_iosemu_emulator_emu_EmulatorBridge_nativeLaunch<'local>(
    ...
    let outcome = session::launch(Path::new(&ipa), Path::new(&containers), Path::new(&system));
...
/// Serialise the app list to a compact JSON array by hand (no serde dependency).
